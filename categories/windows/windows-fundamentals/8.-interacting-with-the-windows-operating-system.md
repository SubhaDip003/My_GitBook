...
# 8. Interacting with the Windows Operating System

## **Graphical User Interface (GUI)**

A **Graphical User Interface (GUI)** is a user-friendly way to interact with a computer using **mouse clicks, icons, and menus** instead of typing commands. It was introduced in the late 1970s to make computers easier to use for people who are not familiar with command-line operations.

GUI made computers more accessible because users no longer needed to remember commands or have programming knowledge. Today, most Windows users rely on GUI for everyday tasks, while system administrators also use it to manage systems like Active Directory, IIS, and databases more easily.

***

## **Remote Desktop Protocol (RDP)**

**Remote Desktop Protocol (RDP)** is a Microsoft protocol that allows a user to remotely connect to another computer over a network and use its graphical interface (GUI) as if they are physically present. The connection is made using an RDP client to a system running an RDP server, and it communicates over port 3389. RDP is commonly used by system administrators to manage remote systems and by users to access their work computers from different locations, often through a VPN.

***

## Windows Command Line <a href="#windows-command-line" id="windows-command-line"></a>
...
## Cmdlets <a href="#cmdlets" id="cmdlets"></a>

PowerShell uses [**cmdlets**](https://learn.microsoft.com/en-us/powershell/scripting/developer/cmdlet/cmdlet-overview?view=powershell-7.6\&viewFallbackFrom=powershell-7), which are small built-in commands designed to perform a single specific task. These cmdlets are lightweight, easy to use, and form the core of PowerShell functionality. There are more than 100 default cmdlets available, and additional cmdlets can be added through modules or even created by users to perform custom tasks. Cmdlets are commonly used for system administration, automation, and scripting.

Cmdlets follow a standard naming format called **Verb-Noun**, which makes them easy to understand. The verb describes the action, and the noun describes the object. For example:

```powershell
Get-ChildItem
```

This command is used to list files and folders in the current directory, similar to `dir` in CMD or `ls` in Linux.

Cmdlets also support **parameters (arguments/flags)** that modify their behavior. You can explore available parameters by typing a command followed by `-` and pressing the **Tab key**.
...
This command lists all files and folders in the current directory, including all subdirectories.
...
This will display the contents of that specific directory.

Parameters can be combined to perform more powerful operations:

```powershell
Get-ChildItem -Path C:\Users\Administrator\Downloads -Recurse
```

This command lists all files and folders inside the Downloads directory and all its subfolders.
...
## Running Scripts <a href="#running-scripts" id="running-scripts"></a>
